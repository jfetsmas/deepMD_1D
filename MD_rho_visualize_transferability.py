...
script__name = 'MD_rho_1D_v7_fulloscillation_py'
alpha = 64
n_train = 100
n_test = 5
intLength = 1280.0
N_near = 1
data_input_prefix = 'KS_MD_scf_128_sigma_6.0'
...
filenameIpt = data_folder + 'Input_'  + data_input_prefix + '.h5'
filenameOpt = data_folder + 'Output_' + data_input_prefix + '.h5'
...
Sample_picked = 1
print(InputArray[Sample_picked,:])
...
# plt.plot(Xpos_picked, Rho_picked,'r')
# plt.show()
...
with tf.Session() as sess:
  ...
  Rho_test_norm = np.sqrt(np.sum(Rho_picked**2))
  Rho_test_err = np.sqrt(np.sum((Rho_picked-Rho_out)**2))/Rho_test_norm
  print('relative error of rho %.3e'% Rho_test_err) 

  #Rho_out = sess.run(Rho, feed_dict= { X: xtest[:,0:1], R: xtest[:,1:], Y: ytest})
  

  plt.plot(Xpos_picked[220:400], Rho_picked[220:400],'r')
  plt.plot(Xpos_picked[220:400], Rho_out[220:400],'b--')
  # plt.plot(Xpos_picked, Rho_out-Rho_picked)
  # plt.savefig(input_prefix+'_time_'+str(Sample_picked)+'MoreSample_Epoch_100.png', bbox_inches="tight")
  plt.savefig(data_input_prefix+'_transferability_zoomin_Nnear_'+str(N_near)+'.png', bbox_inches="tight")
